...
#define span_lite_VERSION                                                                   \
	span_STRINGIFY(span_lite_MAJOR) "." span_STRINGIFY(span_lite_MINOR) "." span_STRINGIFY( \
		span_lite_PATCH)
...
#	else
			...
			struct remove_cv {
				typedef
					typename detail::remove_volatile<typename detail::remove_const<T>::type>::type
						type;
			};
...
#	if span_HAVE(CONSTRAINED_SPAN_CONTAINER_CTOR)
		...
		template<class Container,
			class ElementType span_REQUIRES_T(
...
#	if span_FEATURE(WITH_CONTAINER)
			...
			span_constexpr span(with_container_t, Container const& cont):
				data_(
					cont.size() == 0 ? span_nullptr : const_cast<pointer>(span_ADDRESSOF(cont[0]))),
				size_(to_size(cont.size()))
			{
			}
#	endif
...
#	endif
			...
			span_constexpr_exp span<element_type, dynamic_extent> subspan(index_type offset,
				index_type count = static_cast<index_type>(dynamic_extent)) const
			{
				span_EXPECTS(((0 <= offset && offset <= size())) &&
...
#	if span_HAVE(BYTE) || span_HAVE(NONSTD_BYTE)
		...
		inline span_constexpr span<const detail::byte,
			((Extent == dynamic_extent) ? dynamic_extent : (span_sizeof(T) * Extent))>
...
#		endif
		...
		template<class T, extent_t Extent>
		inline span_constexpr span<detail::byte,
			((Extent == dynamic_extent) ? dynamic_extent : (span_sizeof(T) * Extent))>
...
#	if span_FEATURE(NON_MEMBER_FIRST_LAST_SUB) && span_CPP11_120
		...
		template<class T>
		span_constexpr auto first(T& t, index_t count) -> decltype(make_span(t).first(count))
		{
		...
		template<class T>
		span_constexpr auto subspan(T& t, index_t offset, extent_t count = dynamic_extent)
...
namespace nonstd {
	namespace span_lite {
		...
		inline span_constexpr auto byte_span(T& t) span_noexcept
			-> span<detail::byte, span_sizeof(T)>
		{
			return span<detail::byte, span_sizeof(t)>(
				reinterpret_cast<detail::byte*>(&t), span_sizeof(T));
		}

		template<class T>
		inline span_constexpr auto byte_span(T const& t) span_noexcept
			-> span<const detail::byte, span_sizeof(T)>
		{
			return span<const detail::byte, span_sizeof(t)>(
				reinterpret_cast<detail::byte const*>(&t), span_sizeof(T));
		}
