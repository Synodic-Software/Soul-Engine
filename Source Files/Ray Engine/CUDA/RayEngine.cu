...
namespace RayEngineCUDA {
	...
	__global__ void ExecuteJobs(uint n, Ray* rays, BVHData* bvhP, Vertex* vertices, Face* faces, BoundingBox* boxes, int* counter) {

		const BVHData bvh = *bvhP;
		const uint& nil = bvh.leafSize-1;

		uint traversalStack[STACK_SIZE];
		traversalStack[0] = nil; // Bottom-most entry.

		float eps = exp2f(-80.0f);

		char    stackPtr;                       // Current position in traversal stack.
		uint   currentLeaf;                       // First postponed leaf, non-negative if none.
		uint   currentNode = nil;				// Non-negative: current internal node, negative: second postponed leaf.
		int     rayidx;
		...
		do {
			...
			const bool          terminated = currentNode == nil;
			...
			if (terminated)
			{
				...
				// Setup traversal.
				stackPtr = 0;
				currentLeaf = nil;   // No postponed leaf.
			...
			while (currentNode != nil)
			{
				...
				while (!bvh.IsLeaf(currentNode) && currentNode != nil)
				{
					// Fetch AABBs of the two child nodes.

					const Node& node = bvh.nodes[currentNode];
					uint childL = node.childLeft;
					uint childR = node.childRight;

					const BoundingBox boxLeft = bvh.IsLeaf(node.childLeft) ? boxes[node.childLeft - bvh.leafSize] : bvh.nodes[node.childLeft].box;
					const BoundingBox boxRight = bvh.IsLeaf(node.childRight) ? boxes[node.childRight - bvh.leafSize] : bvh.nodes[node.childRight].box;
				...
				{
					const auto faceID = currentLeaf - bvh.leafSize;
