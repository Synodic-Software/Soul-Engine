...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #855
#include <fstream>
#include <iostream>
#include <string>
#include <string.h>
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #907
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #915
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #940
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #981
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1009
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateInstance(const VkInstanceCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkInstance* pInstance)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateInstance(pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pInstance = " << (void*)*pInstance << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateInstance(pCreateInfo = address, pAllocator = address, pInstance = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
        std::string tmp_str;
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pCreateInfo) {
            tmp_str = vk_print_vkinstancecreateinfo(pCreateInfo, "    ");
            (*outputStream) << "   pCreateInfo (" << pCreateInfo << ")" << std::endl << tmp_str << std::endl;
        }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pAllocator) {
            tmp_str = vk_print_vkallocationcallbacks(pAllocator, "    ");
            (*outputStream) << "   pAllocator (" << pAllocator << ")" << std::endl << tmp_str << std::endl;
        }
    }
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkEnumeratePhysicalDevices(VkInstance instance, uint32_t* pPhysicalDeviceCount, VkPhysicalDevice* pPhysicalDevices)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkEnumeratePhysicalDevices(instance = " << (void*)(instance) << ", pPhysicalDeviceCount = " << *(pPhysicalDeviceCount) << ", pPhysicalDevices = " << (void*)(pPhysicalDevices) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkEnumeratePhysicalDevices(instance = address, pPhysicalDeviceCount = " << *(pPhysicalDeviceCount) << ", pPhysicalDevices = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
        ...
        uint32_t i;
        if (pPhysicalDevices) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < *pPhysicalDeviceCount; i++) {
                tmp_str = string_convert_helper(pPhysicalDevices[i], "    ");
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pPhysicalDevices[" << i << "] (" << &pPhysicalDevices[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pPhysicalDevices[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pFeatures) {
            ...
            (*outputStream) << "   pFeatures (" << pFeatures << ")" << std::endl << tmp_str << std::endl;
        }
    }
    loader_platform_thread_unlock_mutex(&printLock);
}
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pFormatProperties) {
            ...
            (*outputStream) << "   pFormatProperties (" << pFormatProperties << ")" << std::endl << tmp_str << std::endl;
        }
    }
    loader_platform_thread_unlock_mutex(&printLock);
}
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkGetPhysicalDeviceImageFormatProperties(VkPhysicalDevice physicalDevice, VkFormat format, VkImageType type, VkImageTiling tiling, VkImageUsageFlags usage, VkImageCreateFlags flags, VkImageFormatProperties* pImageFormatProperties)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetPhysicalDeviceImageFormatProperties(physicalDevice = " << (void*)(physicalDevice) << ", format = " << string_VkFormat(format) << ", type = " << string_VkImageType(type) << ", tiling = " << string_VkImageTiling(tiling) << ", usage = " << usage << ", flags = " << flags << ", pImageFormatProperties = " << (void*)(pImageFormatProperties) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetPhysicalDeviceImageFormatProperties(physicalDevice = address, format = " << string_VkFormat(format) << ", type = " << string_VkImageType(type) << ", tiling = " << string_VkImageTiling(tiling) << ", usage = " << usage << ", flags = " << flags << ", pImageFormatProperties = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pImageFormatProperties) {
            ...
            (*outputStream) << "   pImageFormatProperties (" << pImageFormatProperties << ")" << std::endl << tmp_str << std::endl;
        }
    }
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pProperties) {
            ...
            (*outputStream) << "   pProperties (" << pProperties << ")" << std::endl << tmp_str << std::endl;
        }
    }
    loader_platform_thread_unlock_mutex(&printLock);
}
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pQueueFamilyProperties) {
            ...
            (*outputStream) << "   pQueueFamilyProperties (" << pQueueFamilyProperties << ")" << std::endl << tmp_str << std::endl;
        }
    }
    loader_platform_thread_unlock_mutex(&printLock);
}
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pMemoryProperties) {
            ...
            (*outputStream) << "   pMemoryProperties (" << pMemoryProperties << ")" << std::endl << tmp_str << std::endl;
        }
    }
    loader_platform_thread_unlock_mutex(&printLock);
}

// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1181
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateDevice(VkPhysicalDevice physicalDevice, const VkDeviceCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkDevice* pDevice)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateDevice(physicalDevice = " << (void*)(physicalDevice) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pDevice = " << (void*)*pDevice << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateDevice(physicalDevice = address, pCreateInfo = address, pAllocator = address, pDevice = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #301
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #328
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #338
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkQueueSubmit(VkQueue queue, uint32_t submitCount, const VkSubmitInfo* pSubmits, VkFence fence)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkQueueSubmit(queue = " << (void*)(queue) << ", submitCount = " << submitCount << ", pSubmits = " << (void*)(pSubmits) << ", fence = " << fence << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkQueueSubmit(queue = address, submitCount = " << submitCount << ", pSubmits = address, fence = " << fence << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < submitCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pSubmits[" << i << "] (" << &pSubmits[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pSubmits[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkQueueWaitIdle(VkQueue queue)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkQueueWaitIdle(queue = " << (void*)(queue) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkQueueWaitIdle(queue = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkDeviceWaitIdle(VkDevice device)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkDeviceWaitIdle(device = " << (void*)(device) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkDeviceWaitIdle(device = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkAllocateMemory(VkDevice device, const VkMemoryAllocateInfo* pAllocateInfo, const VkAllocationCallbacks* pAllocator, VkDeviceMemory* pMemory)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkAllocateMemory(device = " << (void*)(device) << ", pAllocateInfo = " << (void*)(pAllocateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pMemory = " << pMemory << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkAllocateMemory(device = address, pAllocateInfo = address, pAllocator = address, pMemory = " << pMemory << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pAllocateInfo) {
            ...
            (*outputStream) << "   pAllocateInfo (" << pAllocateInfo << ")" << std::endl << tmp_str << std::endl;
        }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pAllocator) {
            tmp_str = vk_print_vkallocationcallbacks(pAllocator, "    ");
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkMapMemory(VkDevice device, VkDeviceMemory memory, VkDeviceSize offset, VkDeviceSize size, VkMemoryMapFlags flags, void** ppData)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkMapMemory(device = " << (void*)(device) << ", memory = " << memory << ", offset = " << (void*)(offset) << ", size = " << (void*)(size) << ", flags = " << flags << ", ppData = " << (void*)*ppData << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkMapMemory(device = address, memory = " << memory << ", offset = address, size = address, flags = " << flags << ", ppData = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkFlushMappedMemoryRanges(VkDevice device, uint32_t memoryRangeCount, const VkMappedMemoryRange* pMemoryRanges)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkFlushMappedMemoryRanges(device = " << (void*)(device) << ", memoryRangeCount = " << memoryRangeCount << ", pMemoryRanges = " << (void*)(pMemoryRanges) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkFlushMappedMemoryRanges(device = address, memoryRangeCount = " << memoryRangeCount << ", pMemoryRanges = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < memoryRangeCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pMemoryRanges[" << i << "] (" << &pMemoryRanges[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pMemoryRanges[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkInvalidateMappedMemoryRanges(VkDevice device, uint32_t memoryRangeCount, const VkMappedMemoryRange* pMemoryRanges)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkInvalidateMappedMemoryRanges(device = " << (void*)(device) << ", memoryRangeCount = " << memoryRangeCount << ", pMemoryRanges = " << (void*)(pMemoryRanges) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkInvalidateMappedMemoryRanges(device = address, memoryRangeCount = " << memoryRangeCount << ", pMemoryRanges = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkBindBufferMemory(VkDevice device, VkBuffer buffer, VkDeviceMemory memory, VkDeviceSize memoryOffset)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkBindBufferMemory(device = " << (void*)(device) << ", buffer = " << buffer << ", memory = " << memory << ", memoryOffset = " << (void*)(memoryOffset) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkBindBufferMemory(device = address, buffer = " << buffer << ", memory = " << memory << ", memoryOffset = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkBindImageMemory(VkDevice device, VkImage image, VkDeviceMemory memory, VkDeviceSize memoryOffset)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkBindImageMemory(device = " << (void*)(device) << ", image = " << image << ", memory = " << memory << ", memoryOffset = " << (void*)(memoryOffset) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkBindImageMemory(device = address, image = " << image << ", memory = " << memory << ", memoryOffset = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pMemoryRequirements) {
            ...
            (*outputStream) << "   pMemoryRequirements (" << pMemoryRequirements << ")" << std::endl << tmp_str << std::endl;
        }
    }
    loader_platform_thread_unlock_mutex(&printLock);
}
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < *pSparseMemoryRequirementCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pSparseMemoryRequirements[" << i << "] (" << &pSparseMemoryRequirements[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pSparseMemoryRequirements[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkQueueBindSparse(VkQueue queue, uint32_t bindInfoCount, const VkBindSparseInfo* pBindInfo, VkFence fence)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkQueueBindSparse(queue = " << (void*)(queue) << ", bindInfoCount = " << bindInfoCount << ", pBindInfo = " << (void*)(pBindInfo) << ", fence = " << fence << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkQueueBindSparse(queue = address, bindInfoCount = " << bindInfoCount << ", pBindInfo = address, fence = " << fence << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < bindInfoCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pBindInfo[" << i << "] (" << &pBindInfo[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pBindInfo[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateFence(VkDevice device, const VkFenceCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkFence* pFence)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateFence(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pFence = " << pFence << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateFence(device = address, pCreateInfo = address, pAllocator = address, pFence = " << pFence << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkResetFences(VkDevice device, uint32_t fenceCount, const VkFence* pFences)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkResetFences(device = " << (void*)(device) << ", fenceCount = " << fenceCount << ", pFences = " << (void*)(pFences) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkResetFences(device = address, fenceCount = " << fenceCount << ", pFences = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < fenceCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pFences[" << i << "] (" << &pFences[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pFences[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkGetFenceStatus(VkDevice device, VkFence fence)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetFenceStatus(device = " << (void*)(device) << ", fence = " << fence << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetFenceStatus(device = address, fence = " << fence << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkWaitForFences(VkDevice device, uint32_t fenceCount, const VkFence* pFences, VkBool32 waitAll, uint64_t timeout)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkWaitForFences(device = " << (void*)(device) << ", fenceCount = " << fenceCount << ", pFences = " << (void*)(pFences) << ", waitAll = " << waitAll << ", timeout = " << timeout << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkWaitForFences(device = address, fenceCount = " << fenceCount << ", pFences = address, waitAll = " << waitAll << ", timeout = " << timeout << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateSemaphore(VkDevice device, const VkSemaphoreCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkSemaphore* pSemaphore)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateSemaphore(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pSemaphore = " << pSemaphore << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateSemaphore(device = address, pCreateInfo = address, pAllocator = address, pSemaphore = " << pSemaphore << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateEvent(VkDevice device, const VkEventCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkEvent* pEvent)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateEvent(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pEvent = " << pEvent << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateEvent(device = address, pCreateInfo = address, pAllocator = address, pEvent = " << pEvent << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkGetEventStatus(VkDevice device, VkEvent event)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetEventStatus(device = " << (void*)(device) << ", event = " << event << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetEventStatus(device = address, event = " << event << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkSetEvent(VkDevice device, VkEvent event)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkSetEvent(device = " << (void*)(device) << ", event = " << event << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkSetEvent(device = address, event = " << event << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkResetEvent(VkDevice device, VkEvent event)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkResetEvent(device = " << (void*)(device) << ", event = " << event << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkResetEvent(device = address, event = " << event << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateQueryPool(VkDevice device, const VkQueryPoolCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkQueryPool* pQueryPool)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateQueryPool(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pQueryPool = " << pQueryPool << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateQueryPool(device = address, pCreateInfo = address, pAllocator = address, pQueryPool = " << pQueryPool << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkGetQueryPoolResults(VkDevice device, VkQueryPool queryPool, uint32_t firstQuery, uint32_t queryCount, size_t dataSize, void* pData, VkDeviceSize stride, VkQueryResultFlags flags)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetQueryPoolResults(device = " << (void*)(device) << ", queryPool = " << queryPool << ", firstQuery = " << firstQuery << ", queryCount = " << queryCount << ", dataSize = " << (unsigned long)dataSize << ", pData = " << (void*)(pData) << ", stride = " << (void*)(stride) << ", flags = " << flags << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetQueryPoolResults(device = address, queryPool = " << queryPool << ", firstQuery = " << firstQuery << ", queryCount = " << queryCount << ", dataSize = " << (unsigned long)dataSize << ", pData = address, stride = address, flags = " << flags << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateBuffer(VkDevice device, const VkBufferCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkBuffer* pBuffer)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateBuffer(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pBuffer = " << pBuffer << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateBuffer(device = address, pCreateInfo = address, pAllocator = address, pBuffer = " << pBuffer << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateBufferView(VkDevice device, const VkBufferViewCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkBufferView* pView)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateBufferView(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pView = " << pView << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateBufferView(device = address, pCreateInfo = address, pAllocator = address, pView = " << pView << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateImage(VkDevice device, const VkImageCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkImage* pImage)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateImage(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pImage = " << pImage << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateImage(device = address, pCreateInfo = address, pAllocator = address, pImage = " << pImage << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pSubresource) {
            ...
            (*outputStream) << "   pSubresource (" << pSubresource << ")" << std::endl << tmp_str << std::endl;
        }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pLayout) {
            tmp_str = vk_print_vksubresourcelayout(pLayout, "    ");
            (*outputStream) << "   pLayout (" << pLayout << ")" << std::endl << tmp_str << std::endl;
        }
    }
    loader_platform_thread_unlock_mutex(&printLock);
}
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateImageView(VkDevice device, const VkImageViewCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkImageView* pView)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateImageView(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pView = " << pView << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateImageView(device = address, pCreateInfo = address, pAllocator = address, pView = " << pView << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateShaderModule(VkDevice device, const VkShaderModuleCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkShaderModule* pShaderModule)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateShaderModule(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pShaderModule = " << pShaderModule << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateShaderModule(device = address, pCreateInfo = address, pAllocator = address, pShaderModule = " << pShaderModule << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreatePipelineCache(VkDevice device, const VkPipelineCacheCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkPipelineCache* pPipelineCache)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreatePipelineCache(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pPipelineCache = " << pPipelineCache << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreatePipelineCache(device = address, pCreateInfo = address, pAllocator = address, pPipelineCache = " << pPipelineCache << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkGetPipelineCacheData(VkDevice device, VkPipelineCache pipelineCache, size_t* pDataSize, void* pData)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetPipelineCacheData(device = " << (void*)(device) << ", pipelineCache = " << pipelineCache << ", pDataSize = " << (unsigned long)*pDataSize << ", pData = " << (void*)(pData) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetPipelineCacheData(device = address, pipelineCache = " << pipelineCache << ", pDataSize = " << (unsigned long)*pDataSize << ", pData = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkMergePipelineCaches(VkDevice device, VkPipelineCache dstCache, uint32_t srcCacheCount, const VkPipelineCache* pSrcCaches)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkMergePipelineCaches(device = " << (void*)(device) << ", dstCache = " << dstCache << ", srcCacheCount = " << srcCacheCount << ", pSrcCaches = " << (void*)(pSrcCaches) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkMergePipelineCaches(device = address, dstCache = " << dstCache << ", srcCacheCount = " << srcCacheCount << ", pSrcCaches = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < srcCacheCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pSrcCaches[" << i << "] (" << &pSrcCaches[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pSrcCaches[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateGraphicsPipelines(VkDevice device, VkPipelineCache pipelineCache, uint32_t createInfoCount, const VkGraphicsPipelineCreateInfo* pCreateInfos, const VkAllocationCallbacks* pAllocator, VkPipeline* pPipelines)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateGraphicsPipelines(device = " << (void*)(device) << ", pipelineCache = " << pipelineCache << ", createInfoCount = " << createInfoCount << ", pCreateInfos = " << (void*)(pCreateInfos) << ", pAllocator = " << (void*)(pAllocator) << ", pPipelines = " << pPipelines << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateGraphicsPipelines(device = address, pipelineCache = " << pipelineCache << ", createInfoCount = " << createInfoCount << ", pCreateInfos = address, pAllocator = address, pPipelines = " << pPipelines << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < createInfoCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pCreateInfos[" << i << "] (" << &pCreateInfos[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pCreateInfos[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateComputePipelines(VkDevice device, VkPipelineCache pipelineCache, uint32_t createInfoCount, const VkComputePipelineCreateInfo* pCreateInfos, const VkAllocationCallbacks* pAllocator, VkPipeline* pPipelines)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateComputePipelines(device = " << (void*)(device) << ", pipelineCache = " << pipelineCache << ", createInfoCount = " << createInfoCount << ", pCreateInfos = " << (void*)(pCreateInfos) << ", pAllocator = " << (void*)(pAllocator) << ", pPipelines = " << pPipelines << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateComputePipelines(device = address, pipelineCache = " << pipelineCache << ", createInfoCount = " << createInfoCount << ", pCreateInfos = address, pAllocator = address, pPipelines = " << pPipelines << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreatePipelineLayout(VkDevice device, const VkPipelineLayoutCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkPipelineLayout* pPipelineLayout)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreatePipelineLayout(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pPipelineLayout = " << pPipelineLayout << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreatePipelineLayout(device = address, pCreateInfo = address, pAllocator = address, pPipelineLayout = " << pPipelineLayout << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateSampler(VkDevice device, const VkSamplerCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkSampler* pSampler)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateSampler(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pSampler = " << pSampler << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateSampler(device = address, pCreateInfo = address, pAllocator = address, pSampler = " << pSampler << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateDescriptorSetLayout(VkDevice device, const VkDescriptorSetLayoutCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkDescriptorSetLayout* pSetLayout)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateDescriptorSetLayout(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pSetLayout = " << pSetLayout << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateDescriptorSetLayout(device = address, pCreateInfo = address, pAllocator = address, pSetLayout = " << pSetLayout << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateDescriptorPool(VkDevice device, const VkDescriptorPoolCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkDescriptorPool* pDescriptorPool)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateDescriptorPool(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pDescriptorPool = " << pDescriptorPool << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateDescriptorPool(device = address, pCreateInfo = address, pAllocator = address, pDescriptorPool = " << pDescriptorPool << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkResetDescriptorPool(VkDevice device, VkDescriptorPool descriptorPool, VkDescriptorPoolResetFlags flags)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkResetDescriptorPool(device = " << (void*)(device) << ", descriptorPool = " << descriptorPool << ", flags = " << flags << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkResetDescriptorPool(device = address, descriptorPool = " << descriptorPool << ", flags = " << flags << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkAllocateDescriptorSets(VkDevice device, const VkDescriptorSetAllocateInfo* pAllocateInfo, VkDescriptorSet* pDescriptorSets)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkAllocateDescriptorSets(device = " << (void*)(device) << ", pAllocateInfo = " << (void*)(pAllocateInfo) << ", pDescriptorSets = " << pDescriptorSets << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkAllocateDescriptorSets(device = address, pAllocateInfo = address, pDescriptorSets = " << pDescriptorSets << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkFreeDescriptorSets(VkDevice device, VkDescriptorPool descriptorPool, uint32_t descriptorSetCount, const VkDescriptorSet* pDescriptorSets)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkFreeDescriptorSets(device = " << (void*)(device) << ", descriptorPool = " << descriptorPool << ", descriptorSetCount = " << descriptorSetCount << ", pDescriptorSets = " << (void*)(pDescriptorSets) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkFreeDescriptorSets(device = address, descriptorPool = " << descriptorPool << ", descriptorSetCount = " << descriptorSetCount << ", pDescriptorSets = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < descriptorSetCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pDescriptorSets[" << i << "] (" << &pDescriptorSets[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pDescriptorSets[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < descriptorWriteCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pDescriptorWrites[" << i << "] (" << &pDescriptorWrites[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pDescriptorWrites[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pDescriptorCopies) {
            ...
            (*outputStream) << "   pDescriptorCopies (" << pDescriptorCopies << ")" << std::endl << tmp_str << std::endl;
        }
    }
    loader_platform_thread_unlock_mutex(&printLock);
}
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateFramebuffer(VkDevice device, const VkFramebufferCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkFramebuffer* pFramebuffer)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateFramebuffer(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pFramebuffer = " << pFramebuffer << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateFramebuffer(device = address, pCreateInfo = address, pAllocator = address, pFramebuffer = " << pFramebuffer << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateRenderPass(VkDevice device, const VkRenderPassCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkRenderPass* pRenderPass)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateRenderPass(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pRenderPass = " << pRenderPass << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateRenderPass(device = address, pCreateInfo = address, pAllocator = address, pRenderPass = " << pRenderPass << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pGranularity) {
            ...
            (*outputStream) << "   pGranularity (" << pGranularity << ")" << std::endl << tmp_str << std::endl;
        }
    }
    loader_platform_thread_unlock_mutex(&printLock);
}
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateCommandPool(VkDevice device, const VkCommandPoolCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkCommandPool* pCommandPool)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateCommandPool(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pCommandPool = " << pCommandPool << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateCommandPool(device = address, pCreateInfo = address, pAllocator = address, pCommandPool = " << pCommandPool << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkResetCommandPool(VkDevice device, VkCommandPool commandPool, VkCommandPoolResetFlags flags)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkResetCommandPool(device = " << (void*)(device) << ", commandPool = " << commandPool << ", flags = " << flags << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkResetCommandPool(device = address, commandPool = " << commandPool << ", flags = " << flags << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkAllocateCommandBuffers(VkDevice device, const VkCommandBufferAllocateInfo* pAllocateInfo, VkCommandBuffer* pCommandBuffers)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkAllocateCommandBuffers(device = " << (void*)(device) << ", pAllocateInfo = " << (void*)(pAllocateInfo) << ", pCommandBuffers = " << (void*)*pCommandBuffers << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkAllocateCommandBuffers(device = address, pAllocateInfo = address, pCommandBuffers = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < commandBufferCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pCommandBuffers[" << i << "] (" << &pCommandBuffers[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pCommandBuffers[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkBeginCommandBuffer(VkCommandBuffer commandBuffer, const VkCommandBufferBeginInfo* pBeginInfo)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkBeginCommandBuffer(commandBuffer = " << (void*)(commandBuffer) << ", pBeginInfo = " << (void*)(pBeginInfo) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkBeginCommandBuffer(commandBuffer = address, pBeginInfo = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pBeginInfo) {
            ...
            (*outputStream) << "   pBeginInfo (" << pBeginInfo << ")" << std::endl << tmp_str << std::endl;
        }
    }
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkEndCommandBuffer(VkCommandBuffer commandBuffer)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkEndCommandBuffer(commandBuffer = " << (void*)(commandBuffer) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkEndCommandBuffer(commandBuffer = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkResetCommandBuffer(VkCommandBuffer commandBuffer, VkCommandBufferResetFlags flags)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkResetCommandBuffer(commandBuffer = " << (void*)(commandBuffer) << ", flags = " << flags << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkResetCommandBuffer(commandBuffer = address, flags = " << flags << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < viewportCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pViewports[" << i << "] (" << &pViewports[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pViewports[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < scissorCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pScissors[" << i << "] (" << &pScissors[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pScissors[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < dynamicOffsetCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pDynamicOffsets[" << i << "] (" << &pDynamicOffsets[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pDynamicOffsets[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < regionCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pRegions[" << i << "] (" << &pRegions[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pRegions[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pColor) {
            ...
            (*outputStream) << "   pColor (" << pColor << ")" << std::endl << tmp_str << std::endl;
        }
        uint32_t i;
        if (pRanges) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < rangeCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pRanges[" << i << "] (" << &pRanges[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pRanges[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pDepthStencil) {
            ...
            (*outputStream) << "   pDepthStencil (" << pDepthStencil << ")" << std::endl << tmp_str << std::endl;
        }
        uint32_t i;
        if (pRanges) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < attachmentCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pAttachments[" << i << "] (" << &pAttachments[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pAttachments[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
        if (pRects) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < rectCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pRects[" << i << "] (" << &pRects[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pRects[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < bufferMemoryBarrierCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pBufferMemoryBarriers[" << i << "] (" << &pBufferMemoryBarriers[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pBufferMemoryBarriers[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
        if (pEvents) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < eventCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pEvents[" << i << "] (" << &pEvents[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pEvents[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < imageMemoryBarrierCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pImageMemoryBarriers[" << i << "] (" << &pImageMemoryBarriers[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pImageMemoryBarriers[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
        if (pMemoryBarriers) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < memoryBarrierCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pMemoryBarriers[" << i << "] (" << &pMemoryBarriers[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pMemoryBarriers[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pRenderPassBegin) {
            ...
            (*outputStream) << "   pRenderPassBegin (" << pRenderPassBegin << ")" << std::endl << tmp_str << std::endl;
        }
    }
    loader_platform_thread_unlock_mutex(&printLock);
}
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkGetPhysicalDeviceSurfaceSupportKHR(VkPhysicalDevice physicalDevice, uint32_t queueFamilyIndex, VkSurfaceKHR surface, VkBool32* pSupported)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetPhysicalDeviceSurfaceSupportKHR(physicalDevice = " << (void*)(physicalDevice) << ", queueFamilyIndex = " << queueFamilyIndex << ", surface = " << surface << ", pSupported = " << pSupported << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetPhysicalDeviceSurfaceSupportKHR(physicalDevice = address, queueFamilyIndex = " << queueFamilyIndex << ", surface = " << surface << ", pSupported = " << pSupported << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkGetPhysicalDeviceSurfaceCapabilitiesKHR(VkPhysicalDevice physicalDevice, VkSurfaceKHR surface, VkSurfaceCapabilitiesKHR* pSurfaceCapabilities)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDevice = " << (void*)(physicalDevice) << ", surface = " << surface << ", pSurfaceCapabilities = " << (void*)(pSurfaceCapabilities) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDevice = address, surface = " << surface << ", pSurfaceCapabilities = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pSurfaceCapabilities) {
            ...
            (*outputStream) << "   pSurfaceCapabilities (" << pSurfaceCapabilities << ")" << std::endl << tmp_str << std::endl;
        }
    }
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkGetPhysicalDeviceSurfaceFormatsKHR(VkPhysicalDevice physicalDevice, VkSurfaceKHR surface, uint32_t* pSurfaceFormatCount, VkSurfaceFormatKHR* pSurfaceFormats)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice = " << (void*)(physicalDevice) << ", surface = " << surface << ", pSurfaceFormatCount = " << *(pSurfaceFormatCount) << ", pSurfaceFormats = " << (void*)(pSurfaceFormats) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice = address, surface = " << surface << ", pSurfaceFormatCount = " << *(pSurfaceFormatCount) << ", pSurfaceFormats = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < *pSurfaceFormatCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pSurfaceFormats[" << i << "] (" << &pSurfaceFormats[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pSurfaceFormats[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkGetPhysicalDeviceSurfacePresentModesKHR(VkPhysicalDevice physicalDevice, VkSurfaceKHR surface, uint32_t* pPresentModeCount, VkPresentModeKHR* pPresentModes)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice = " << (void*)(physicalDevice) << ", surface = " << surface << ", pPresentModeCount = " << *(pPresentModeCount) << ", pPresentModes = " << (void*)(pPresentModes) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice = address, surface = " << surface << ", pPresentModeCount = " << *(pPresentModeCount) << ", pPresentModes = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < *pPresentModeCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pPresentModes[" << i << "] (" << &pPresentModes[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pPresentModes[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateSwapchainKHR(VkDevice device, const VkSwapchainCreateInfoKHR* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkSwapchainKHR* pSwapchain)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateSwapchainKHR(device = " << (void*)(device) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pSwapchain = " << pSwapchain << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateSwapchainKHR(device = address, pCreateInfo = address, pAllocator = address, pSwapchain = " << pSwapchain << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkGetSwapchainImagesKHR(VkDevice device, VkSwapchainKHR swapchain, uint32_t* pSwapchainImageCount, VkImage* pSwapchainImages)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetSwapchainImagesKHR(device = " << (void*)(device) << ", swapchain = " << swapchain << ", pSwapchainImageCount = " << *(pSwapchainImageCount) << ", pSwapchainImages = " << pSwapchainImages << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkGetSwapchainImagesKHR(device = address, swapchain = " << swapchain << ", pSwapchainImageCount = " << *(pSwapchainImageCount) << ", pSwapchainImages = " << pSwapchainImages << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1130
            for (i = 0; i < *pSwapchainImageCount; i++) {
                ...
                if (StreamControl::writeAddress == true) {
                    (*outputStream) << "   pSwapchainImages[" << i << "] (" << &pSwapchainImages[i] << ")" << std::endl << tmp_str << std::endl;
                } else {
                    (*outputStream) << "   pSwapchainImages[" << i << "] (address)" << std::endl << "    address" << std::endl;
                }
            }
        }
    }
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkAcquireNextImageKHR(VkDevice device, VkSwapchainKHR swapchain, uint64_t timeout, VkSemaphore semaphore, VkFence fence, uint32_t* pImageIndex)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkAcquireNextImageKHR(device = " << (void*)(device) << ", swapchain = " << swapchain << ", timeout = " << timeout << ", semaphore = " << semaphore << ", fence = " << fence << ", pImageIndex = " << *(pImageIndex) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkAcquireNextImageKHR(device = address, swapchain = " << swapchain << ", timeout = " << timeout << ", semaphore = " << semaphore << ", fence = " << fence << ", pImageIndex = " << *(pImageIndex) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkQueuePresentKHR(VkQueue queue, const VkPresentInfoKHR* pPresentInfo)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkQueuePresentKHR(queue = " << (void*)(queue) << ", pPresentInfo = " << (void*)(pPresentInfo) << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkQueuePresentKHR(queue = address, pPresentInfo = address) = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1107
        if (pPresentInfo) {
            ...
            (*outputStream) << "   pPresentInfo (" << pPresentInfo << ")" << std::endl << tmp_str << std::endl;
        }
    }
    loader_platform_thread_unlock_mutex(&printLock);
    return result;
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateWin32SurfaceKHR(VkInstance instance, const VkWin32SurfaceCreateInfoKHR* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkSurfaceKHR* pSurface)
{
    ...
    // CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1039
    if (StreamControl::writeAddress == true) {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateWin32SurfaceKHR(instance = " << (void*)(instance) << ", pCreateInfo = " << (void*)(pCreateInfo) << ", pAllocator = " << (void*)(pAllocator) << ", pSurface = " << pSurface << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
    else {
        (*outputStream) << "t{" << getTIDIndex() << "} vkCreateWin32SurfaceKHR(instance = address, pCreateInfo = address, pAllocator = address, pSurface = " << pSurface << ") = " << string_VkResult((VkResult)result) << std::endl;
    }
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1091
    if (g_ApiDumpDetailed) {
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #1098
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #410
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #544
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #582
    {
...
// CODEGEN : file C:/releasebuild/VulkanTools/vk-vtlayer-generate.py line #589
