...
/////////////////////////Variables and Declarations//////////////////
	...
	struct VulkanVert {
		...
		static VkVertexInputBindingDescription getBindingDescription() {
			...
			bindingDescription.stride = sizeof(VulkanVert);
		...
		static std::array<VkVertexInputAttributeDescription, 3> getAttributeDescriptions() {
			...
			attributeDescriptions[0].offset = offsetof(VulkanVert, pos);

			attributeDescriptions[1].binding = 0;
			attributeDescriptions[1].location = 1;
			attributeDescriptions[1].format = VK_FORMAT_R32G32B32_SFLOAT;
			attributeDescriptions[1].offset = offsetof(VulkanVert, color);

			attributeDescriptions[2].binding = 0;
			attributeDescriptions[2].location = 2;
			attributeDescriptions[2].format = VK_FORMAT_R32G32_SFLOAT;
			attributeDescriptions[2].offset = offsetof(VulkanVert, texCoord);

			return attributeDescriptions;
		}

		bool operator==(const VulkanVert& other) const {
...
struct UniformBufferObject {
	...
	void createSurface(int window) {
		if (glfwCreateWindowSurface(instance, windows[window], nullptr, &vulkanSurface) != VK_SUCCESS) {
			throw std::runtime_error("failed to create window vulkanSurface!");
		}
	}

	void createSwapChain() {
		SwapChainSupportDetails swapChainSupport = querySwapChainSupport(vulkanDevicesPhysical);
	...
	void createCommandPool() {
		QueueFamilyIndices queueFamilyIndices = findQueueFamilies(vulkanDevicesPhysical);
	...
	VkFormat findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) {
		for (VkFormat format : candidates) {
			...
			vkGetPhysicalDeviceFormatProperties(vulkanDevicesPhysical, format, &props);
	...
	uint32_t findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
		...
		vkGetPhysicalDeviceMemoryProperties(vulkanDevicesPhysical, &memProperties);
