...
//}
//
//void VulkanBackend::AddWindow(GLFWwindow* window, int width, int height,std::vector<void*>& imageData){
//
//	windows.push_back(window);
//	if (windows.size() == 1){
//		swapImageData = imageData;
...
//	CreateInstance();
//	SetupDebugCallback();
//	CreateSurface(windows[0]);
//	PickVulkanDevice();
//	CreateVulkanLogical();
//	CreateSwapChain(windows[0]);
...
//	app->RecreateSwapChain(window, width, height);
//}
//
//void VulkanBackend::IdleDevice(){
//	vkDeviceWaitIdle(device);
//}
//
//void VulkanBackend::RecreateSwapChain(GLFWwindow * window, int width, int height) {
//	vkDeviceWaitIdle(device);
//
...
//}
//
//void VulkanBackend::PickVulkanDevice() {
...
//}
//
//void VulkanBackend::CreateVulkanLogical() {
...
//	swapChainReal.resize(imageCount);
//	swapChainImages.resize(imageCount);
//	vkGetSwapchainImagesKHR(device, swapChain, &imageCount, swapChainReal.data());
//
//	swapChainImageFormat = surfaceFormat.format;
//	swapChainExtent = extent;
//}
//
//void VulkanBackend::CreateImageViews(int width,int height) {
//	swapChainImageViews.resize(swapChainReal.size(), VulkanWrapper<VkImageView>{device, vkDestroyImageView});
//	textureImageMemory.resize(swapChainReal.size(), VulkanWrapper<VkDeviceMemory>{device, vkFreeMemory});
//
...
//
//
//		vkMapMemory(device, textureImageMemory[i], 0, width * height * 4, 0, &swapImageData[i]);
//		vkUnmapMemory(device, textureImageMemory[i]);
//
//		CreateImage(width, height, swapChainImageFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT | VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT, swapChainImages[i], textureImageMemory[i]);
//
//		
...
//
//	VulkanWrapper<VkShaderModule> vertShaderModule{ device, vkDestroyShaderModule };
//	VulkanWrapper<VkShaderModule> fragShaderModule{ device, vkDestroyShaderModule };
...
//	swapChainFramebuffers.resize(swapChainImageViews.size(), VulkanWrapper<VkFramebuffer>{device, vkDestroyFramebuffer});
//
...
////	unsigned char* pixels = DirectLoad("SoulDefault.png", &texWidth, &texHeight, &texChannels);
...
////	}
////
////	VulkanWrapper<VkImage> stagingImage{ device, vkDestroyImage };
////	VulkanWrapper<VkDeviceMemory> stagingImageMemory{ device, vkFreeMemory };
////	CreateImage(texWidth, texHeight, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_LINEAR, VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingImage, stagingImageMemory);
////
////	void* data;
...
////}
//
//void VulkanBackend::CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, VulkanWrapper<VkImageView>& imageView) {
...
//}
//
//void VulkanBackend::CreateCudaImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VulkanWrapper<VkImage>& image, VulkanWrapper<VkDeviceMemory>& imageMemory) {
//
//	VkExternalMemoryImageCreateInfoNV externalImageInfo = {};
//	externalImageInfo.sType = VK_STRUCTURE_TYPE_EXTERNAL_MEMORY_IMAGE_CREATE_INFO_NV;
//	externalImageInfo.pNext = nullptr;
//	externalImageInfo.handleTypes = VK_EXTERNAL_MEMORY;
//
//
...
//
//	VkExportMemoryAllocateInfoNV allocExternalInfo = {};
//	allocExternalInfo.sType = VK_STRUCTURE_TYPE_EXPORT_MEMORY_ALLOCATE_INFO_NV;
//	allocExternalInfo.pNext = nullptr;
//	allocExternalInfo.handleTypes = VK_EXTERNAL_MEMORY;
//
//	VkMemoryAllocateInfo allocInfo = {};
//	allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
//	allocInfo.pNext = &allocExternalInfo;
...
//}
//
//void VulkanBackend::CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VulkanWrapper<VkImage>& image, VulkanWrapper<VkDeviceMemory>& imageMemory) {
//	VkImageCreateInfo imageInfo = {};
//	imageInfo.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
//	imageInfo.imageType = VK_IMAGE_TYPE_2D;
//	imageInfo.extent.width = width;
...
//}
//
//void VulkanBackend::CopyImage(VkImage srcImage, VkImage dstImage, uint32_t width, uint32_t height) {
//	VkCommandBuffer commandBuffer = beginSingleTimeCommands();
//
...
//
//	VulkanWrapper<VkBuffer> stagingBuffer{ device, vkDestroyBuffer };
//	VulkanWrapper<VkDeviceMemory> stagingBufferMemory{ device, vkFreeMemory };
...
//}
//
//void VulkanBackend::CreateUniformBuffer() {
...
//}
//
//void VulkanBackend::CreateDescriptorSet() {
...
//}
//
//void VulkanBackend::CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VulkanWrapper<VkBuffer>& buffer, VulkanWrapper<VkDeviceMemory>& bufferMemory) {
...
//}
//
//void VulkanBackend::UpdateVulkanProjection() {
...
//}
//
//void VulkanBackend::DrawFrame(GLFWwindow * window, int width, int height) {
//	uint32_t imageIndex;
...
//}
//
//void VulkanBackend::CreateShaderModule(const std::vector<char>& code, VulkanWrapper<VkShaderModule>& shaderModule) {
